...
const sanitizeData = (data) => {
  ...
  const sanitized = data.map(item => sanitize(item)).filter(item => item !== null);
  console.log('SanitizeData Input:', data);
  console.log('SanitizeData Output:', sanitized);
  return sanitized;
};
...
function SearchPage() {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      ...
      try {
        ...
        setFeaturedStudies(sanitizeData(featuredStudiesData) || []);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchActiveTrialsData = async () => {
      ...
      try {
        ...
        const sanitizedTrials = sanitizeData(trials);
        console.log('Sanitized Trials:', sanitizedTrials);
        setActiveTrials(prev => {
          const updatedTrials = [...prev, ...sanitizedTrials];
        ...
        console.error('Error fetching active trials:', error.response ? error.response.data : error.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setSearchResults(sanitizeData(response.data) || []);
    } catch (error) {
      console.error('Error searching trials:', error.response ? error.response.data : error.message);
